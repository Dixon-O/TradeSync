/**
 * Sales / POS Page v2
 * Shared inventory, RBAC-aware, with receipt generation.
 */
import { getProducts, hasPrivilege } from '../db/database.js';
...
export async function renderSales(container, profileId, profile) {
    if (!hasPrivilege(profile, 'canSell')) {
        container.innerHTML = `<div class="page active"><div class="empty-state"><h3>Access Denied</h3><p>You don't have permission to make sales</p></div></div>`;
        return;
    }
    ...
    document.getElementById('checkout-btn').addEventListener('click', () => {
        ...
        showCheckoutModal(_cart, profileId, profile?.name || 'Staff', () => {
            _cart = [];
            renderSales(container, profileId, profile);
        });
    });
